...
/// مزود المصادقة
/// تم التصحيح: تحسين التزامن مع الباك اند وإصلاح معالجة الأخطاء
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> _syncWithBackend(
    ...
    try {
      ...
      // إضافة كلمة المرور فقط عند التسجيل بالبريد
      ...
      // تحسين رسالة الخطأ
      if (action == 'login' && 
          response['message']?.toString().contains('not found') == true) {
        _error = 'الحساب غير مسجل في خوادمنا';
      }
    ...
    try {
      ...
      try {
        ...
        if (e.code == 'account-exists-with-different-credential') {
          _error = 'هذا الإيميل مسجل مسبقاً بطريقة أخرى';
          return false;
        }
        rethrow;
      }
  ...
  Future<bool> updateProfile({required String name, String? phone}) async {
    ...
    try {
      ...
      _error = 'خطأ في تحديث الملف الشخصي: ${e.toString()}';
      return false;
    } finally {
      _setLoading(false);
    }
